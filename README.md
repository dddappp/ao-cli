...
> **📋 Inbox机制说明**：Inbox 是进程内部的全局变量，记录所有接收到的但未被handlers消费的消息。成功被handler处理的消息不会留在Inbox中。这是AO系统的核心设计。

> **🔍 --trace 功能说明**：`eval --trace` 显示发送消息的追踪信息。由于AO进程隔离和Inbox机制的限制，目前无法获取接收进程Handler中的print输出。
>
> **注意**：如果进程ID以 `-` 开头，您可以使用以下任一种方法：

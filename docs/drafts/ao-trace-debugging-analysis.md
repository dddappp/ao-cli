...
**关键发现**：在双进程通信中，响应消息的Reference直接等于原始消息的Reference（重用策略），而在单进程通信中，响应消息获得新的Reference（递增策略）。
...
**验证完成**：X-Reference标签确实存在于AO系统中！

**经过验证**：
- ✅ **AO Lua代码中存在X-Reference**：在`ao.send()`、`message.onReply()`和`message.receive()`中使用
...
**最关键的发现**：Reference重用策略取决于通信模式，导致查找复杂度差异！

#### 真正的核心问题
- **通信模式差异**：双进程 vs 单进程通信的Reference重用策略不同
- **双进程通信**：响应消息重用原始Reference（设计行为），查找简单

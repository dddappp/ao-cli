...
### 重要发现
**结论：接收进程 Handlers 中的 print 输出不会被 eval 命令捕获！**

#### 原因分析
1. **异步执行**：`ao.send()` 是异步操作，eval 命令在 Send 后立即返回
2. **执行上下文分离**：
   - eval 在发送进程中执行，捕获发送进程的输出
   - Handler 在接收进程中异步执行，其输出独立
3. **生命周期不同**：Handler 的执行独立于 eval 的生命周期
...
`eval --trace` 是 AO CLI 的最新功能，允许在执行 `eval + Send` 操作时，同时追踪和显示接收进程 Handler 中的 print 输出，实现真正的跨进程调试体验。

### 核心特性

- **🔍 跨进程追踪**：不仅显示发送进程的输出，还能获取接收进程Handler的print输出
- **📊 双模式支持**：支持人类可读的文本模式和结构化的JSON模式
- **🔗 完整链路**：从消息发送到Handler执行的完整调试信息
- **⚡ 实时获取**：通过AO网络API实时获取消息处理结果
...
- **数据大小**：Handler的print输出会增加结果数据的大小
